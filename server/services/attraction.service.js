...
const getAllAttractions = async (options = {}) => {
    let query = Attraction.find({ m_id: { $ne: 0 } }).select('-_id');
    ...
    if (options.sort && typeof options.sort === 'object') {
        ...
        const allowedSortFields = ['m_id', 'title'];
        if (!allowedSortFields.includes(options.sort)) {
            throw new Error(`Недопустимое поле для сортировки: ${options.sort}`);
        }

        const sortDirection = options.order === 'desc' ? -1 : 1;
        query = query.sort({ [options.sort]: sortDirection });
    }
